...
- Docker Hub builds: `./build.sh` / `./build-testing.sh` with optional untracked `build.env` / `build-testing.env` (see `*.env.example`). Local registry: `./build-testing-local.sh` and `build-testing-local.env`. Details: [`README.md` — Build and Update](https://github.com/marc0janssen/nzbgetvpn/blob/develop/README.md#build-and-update).
