...
show_help() {
	cat <<EOF
Usage: $0 [nzbget-testing-version|newest] [--base <tag|newest>] [--docker-repo <namespace/name>] [--platform <platforms>]
       $0 [nzbget-testing-version|newest] --sha256 <expected-sha256> [--base <tag|newest>] [--docker-repo <namespace/name>] [--platform <platforms>]
       $0 [nzbget-testing-version|newest] --accept-downloaded-sha256 [--base <tag|newest>] [--docker-repo <namespace/name>] [--platform <platforms>]
...
Optional env file build-testing.env (copy from build-testing.env.example):
DOCKER_IMAGE_REPO, BUILD_PLATFORM. Optional CLI: --docker-repo, --platform (highest precedence).
...
_env_file="${_script_dir}/build-testing.env"
if [ -f "${_env_file}" ]; then
...
docker buildx build --no-cache --platform "${BUILD_PLATFORM}" --push --build-arg "NZBGETVPN_VERSION=${IMAGE_VERSION}" -t "${DOCKER_IMAGE_REPO}:${VERSION}" -t "${DOCKER_IMAGE_REPO}:${VERSION}-image-v${IMAGE_VERSION}" -t "${DOCKER_IMAGE_REPO}:testing" -f ./Dockerfile-testing .
docker pushrm --file README-containers.md "${DOCKER_IMAGE_REPO}:testing"
